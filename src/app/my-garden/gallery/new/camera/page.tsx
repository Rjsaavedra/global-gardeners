...
export default function CameraPage() {
  ...
  const handleGallerySelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    let shouldNavigateToGalleryNew = false;

    try {
      const dataUrls = await Promise.all(files.map((file) => fileToDataUrl(file)));
      const rawDraftPhotos = sessionStorage.getItem("ggDraftGalleryPhotos");
      let existing: string[] = [];
      ...
      sessionStorage.setItem("ggDraftGalleryPhotos", JSON.stringify(merged));
      shouldNavigateToGalleryNew = true;
    } catch {
      setErrorMessage("Unable to load selected photos.");
    } finally {
      event.target.value = "";
      if (shouldNavigateToGalleryNew) {
        router.push("/my-garden/gallery/new");
      }
    }
  };
  ...
  const handleCapture = () => {
    ...
    sessionStorage.setItem("ggCapturedGalleryPhoto", capturedDataUrl);
    router.push("/my-garden/gallery/new/confirm");
  };

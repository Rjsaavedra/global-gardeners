...
export default function EditPlantUpdatePage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setSelectedDate(found.date || todayIso);
      const dt = found.date ? new Date(`${found.date}T00:00:00`) : new Date();
      ...
      <section className="client-shell relative mx-auto flex min-h-screen w-full max-w-[390px] flex-col overflow-hidden border border-[#e7e0d2] bg-[#f8f6f1]">
        <header className="flex items-center border-b border-black/10 bg-white p-4">
          ...
          <div className="min-w-0 flex-1 pr-10 text-center">
            <h1 className="text-[24px] font-semibold leading-[28.8px] tracking-[-1px] text-[#457941]">Edit post</h1>
          </div>
        </header>
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-[120px] pt-8">
          <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                >
                  ...
                  <span className="relative z-10 text-center">
                    ...
                    <span className="mx-auto block w-[115px] text-[12px] font-normal leading-4 text-[#333333cc]">Take or choose a photo to share.</span>
                  </span>
                </button>
            ...
            <div className="space-y-2">
              <p className="text-[14px] font-semibold leading-5 text-[#333333]">Note (optional)</p>
              <textarea
                placeholder="Write a caption..."
                value={note}
                onChange={(event) => setNote(event.target.value)}
                className="h-[76px] w-full resize-none rounded-[8px] border border-[#e5e5e5] bg-white p-2 text-[14px] font-normal leading-5 text-[#333333] placeholder:text-[#33333380] shadow-[0px_1px_2px_0px_rgba(0,0,0,0.05)] outline-none"
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <span className="text-[14px] font-medium leading-5 text-[#333333]">{displayDate}</span>
              </button>
        ...
        <div className="fixed bottom-0 left-1/2 z-40 w-full max-w-[390px] -translate-x-1/2 bg-[#f8f6f1] px-4 pb-4 pt-5">
          <button
            ...
            onClick={() => {
              ...
              try {
                ...
                const next = list.map((item) =>
                  item.id === params.id
                    ? { ...item, photos: selectedPhotos, photo: selectedPhotos[0] || "", note: note.trim(), date: selectedDate }
                    : item,
                );
            ...
            className={`h-[52px] w-full rounded-[1000px] text-[14px] font-medium leading-5 text-[#fafafa] ${canSave ? "bg-[#457941]" : "bg-[#457941]/50"}`}
          >
            Save
          </button>
        </div>
